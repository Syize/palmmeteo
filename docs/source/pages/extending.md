...
:py:mod:`palmmeteo`. All plugins are placed in separate Python packages with names
starting with `palmmeteo_`. When PALM-meteo starts, it locates all
available packages of such names and treats them as plugins. The included
Python package :py:mod:`palmmeteo_stdplugins` contains the built-in plugins for the
...
:py:class:`palmmeteo_stdplugins.wrf.WRFPlugin`
as your reference.
...
supplied abstract base classes as a base class for your plugin:
 ...
 - :py:class:`palmmeteo.plugins.SetupPluginMixin`
 - :py:class:`palmmeteo.plugins.ImportPluginMixin`
 - :py:class:`palmmeteo.plugins.HInterpPluginMixin`
 - :py:class:`palmmeteo.plugins.VInterpPluginMixin`
 - :py:class:`palmmeteo.plugins.WritePluginMixin`
...
This is an instance of the class :py:class:`palmmeteo.config.ConfigObj` and its
...
The :py:mod:`palmmeteo.logging` module provides these logging functions:

 - :py:func:`palmmeteo.logging.verbose`:
   Prints a verbose message into `sys.stdout` only when `--verbose` is enabled.
 - :py:func:`palmmeteo.logging.log`:
   Prints a standard logging message into `sys.stdout` unless `--silent` is used.
 - :py:func:`palmmeteo.logging.warn`:
   Prints a non-fatal warning message into `sys.stderr`.
 - :py:func:`palmmeteo.logging.die`:
   Prints an error message into `sys.stderr` and terminates the program immediately.
...
:py:class:`palmmeteo.runtime.RuntimeObj`), which is imported as
...
module :py:mod:`palmmeteo.library`. See the code of standard
